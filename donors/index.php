...
// If a donor is already logged in, redirect them to their dashboard
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    // --- DATABASE LOGIN LOGIC FOR DONORS ---
    // Prepare SQL to find a user with the given email AND the 'donor' role
    $stmt = $conn->prepare("SELECT id, password FROM users WHERE email = ? AND role = 'donor'");
    ...
    if ($result->num_rows === 1) {
        ...
        if (password_verify($submitted_password, $user['password'])) {
            ...
            $_SESSION['donor_logged_in'] = true;
            $_SESSION['user_id'] = $user['id'];
            
            // Redirect the donor to their dashboard
            header('Location: dashboard.php');
...
<head>
    ...
    <!-- Link to the CSS file named to match your other login pages -->
    <link rel="stylesheet" href="login_style.css">
</head>
<body>

    <div class="login-container">
        <h2>Donor Portal Login</h2>
        ...
        <div class="form-links">
            Want to help? <a href="register.php">Register as a Donor</a>
        </div>
