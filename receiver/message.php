...
// Get donor info for this request (the accepted donor)
$query = "SELECT 
          ...
          WHERE req.id = ? AND rec.user_id = ? AND res.status = 'Accepted'";
...
if (!$chat_data) {
    die("Chat not found or no accepted donor for this request");
}

$partner_user_id = $chat_data['donor_user_id'];
...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        body { font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; background: linear-gradient(135deg, #14d3b9ff 0%, #583afdff 100%); min-height: 100vh; padding: 20px; }
        ...
        .chat-header { background: linear-gradient(135deg, #3fda39ff 0%, #19bdd3ff 100%); color: white; padding: 20px; display: flex; align-items: center; gap: 15px; }
        ...
        .sent { background: linear-gradient(135deg, #f093fb 0%, #f5576c 100%); color: white; align-self: flex-end; border-bottom-right-radius: 4px; }
        ...
        .chat-input input:focus { outline: none; border-color: #71f557ff; }
        .chat-input button { background: linear-gradient(135deg, #f093fb 0%, #f5576c 100%); color: white; border: none; padding: 12px 25px; border-radius: 25px; cursor: pointer; font-size: 14px; font-weight: 600; transition: transform 0.2s, box-shadow 0.2s; }
        .chat-input button:hover { transform: translateY(-2px); box-shadow: 0 5px 15px rgba(245, 87, 108, 0.4); }
    ...
    <div class="chat-container">
        ...
        <div class="chat-info-bar">
            ...
            <div class="info-item">
                ...
                <span><?php echo htmlspecialchars($chat_data['age']); ?> years old</span>
            </div>
            <div class="info-item">
                <i class="fas fa-vial"></i>
                <span><?php echo htmlspecialchars($chat_data['quantity_ml']); ?> ml needed</span>
            </div>
        </div>
