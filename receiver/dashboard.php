...
if (isset($_GET['cancel_id'])) {
    ...
    // Make sure this request belongs to this recipient
    $check_query = "SELECT id FROM requests WHERE id = ? AND recipient_id = ? AND status = 'Open'";
    $stmt = $conn->prepare($check_query);
    $stmt->bind_param("ii", $cancel_id, $recipient_id);
    $stmt->execute();
    $check_result = $stmt->get_result();

    if ($check_result->num_rows > 0) {
        $update_query = "UPDATE requests SET status = 'Closed' WHERE id = ?";
        $stmt2 = $conn->prepare($update_query);
        $stmt2->bind_param("i", $cancel_id);
        $stmt2->execute();
        $stmt2->close();
        header("Location: admin.php?msg=cancel_success");
        exit();
    }
    $stmt->close();
}
        ...
        <div class="dashboard-content">
            ...
            <div class="section-card">
                ...
                <div class="profile-grid">
                    <div class="profile-item"><strong>Full Name:</strong>
            ...
            <div class="section-card">
                            ...
                            <tbody>
                                <?php while ($request = $my_requests_result->fetch_assoc()): ?>
                                    <tr>
                                        ...
                                        <td><span class="status-badge status-<?php echo strtolower($request['status']); ?>">
                                                <?php echo $request['status']; ?>
                                            </span></td>
                                        <td>
                                            <?php if ($request['status'] == 'Open'): ?>
                                                <a href="admin.php?cancel_id=<?php echo $request['id']; ?>" 
                ...
                <?php else: ?>
                    <div class="empty-state">
                        ...
                        <button class="btn btn-primary" onclick="window.location.href='create_request.php'">
                            Create Request
                        </button>
                    </div>
                <?php endif; ?>
                        ...
                        <?php while ($match = $matches_result->fetch_assoc()): ?>
                            <div class="donor-card">
                                ...
                                <div class="donor-info">
                                    ...
                                    <p><i class="fas fa-calendar"></i> Responded: <?php echo date('M d, Y', strtotime($match['response_date'])); ?></p>
                                    <p><i class="fas fa-info-circle"></i> Status: <?php echo $match['response_status']; ?></p>
                                </div>
